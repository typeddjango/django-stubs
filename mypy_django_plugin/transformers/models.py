    ...
    def run_with_model_cls(self, model_cls: Type[Model]) -> None:
        ...
        for manager_name, manager in model_cls._meta.managers_map.items():
            ...
            if manager_name not in self.model_classdef.info.names:
                ...
                if isinstance(type_now, AnyType) and type_now.type_of_any == TypeOfAny.implementation_artifact:
                    continue
                ...
                is_dynamically_generated = manager_info.metadata.get("django", {}).get("from_queryset_manager")
                if not self.has_any_parametrized_manager_as_base(manager_info) or is_dynamically_generated:
                    manager_type = Instance(
                        manager_info,
                        [Instance(self.model_classdef.info, [])] if manager_info.type_vars else [],
                    )
                    self.add_new_node_to_model_class(manager_name, manager_type)
                    continue
